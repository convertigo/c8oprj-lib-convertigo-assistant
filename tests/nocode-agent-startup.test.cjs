...
const { blockWith, backendParts } = require('./helpers/assistant-source.cjs');
...
test('Forms focus is sent while running and not repeated on completion', () => {
  const generated = blockWith('_c8oProject/mobilePages/Page.yaml', '// Notify saved forms before the running-state early return.');
...
test('Forms context button has readable light and dark colors', () => {
  const scss = blockWith('_c8oProject/mobilePages/Page.yaml', '/* Secondary context action:');
...
test('NoCode uses the deployed bridge library and surfaces engine errors; Studio defaults remain unchanged', () => {
  const snippets = backendParts(['defaultBridgeUrl', 'normalizeSkillProfile', 'assistantProfileDescriptor', 'bridgeCall', 'shouldAttachMcpTokenHandle'], ['DEFAULT_BRIDGE_PROJECT', 'ASSISTANT_PROFILE_BOOTSTRAP']);
  const calls = [];
  ...
  assert.equal(context.defaultBridgeUrl({ userId: 'forms-user', skillProfile: 'nocode' }), 'https://engine/convertigo/projects/lib_ConvertigoAgentBridge/.json');
  assert.equal(context.defaultBridgeUrl({ userId: 'studio', skillProfile: 'generalist' }), 'https://engine/convertigo/projects/lib_ConvertigoAgentBridge/.json');
  context.bridgeCall({ userId: 'forms-user', skillProfile: 'nocode', bridgeBaseUrl: 'https://custom/bridge' }, 'agent_settings', {});
  assert.equal(calls[0][0], 'https://custom/bridge');
  response = { document: { error: { message: 'Project missing' } } };
  assert.throws(() => context.bridgeCall({ userId: 'forms-user', skillProfile: 'nocode' }, 'agent_settings', {}), /Project missing/);
  assert.equal(context.bridgeCall({ userId: 'studio', skillProfile: 'generalist' }, 'agent_settings', {}), response);
});
