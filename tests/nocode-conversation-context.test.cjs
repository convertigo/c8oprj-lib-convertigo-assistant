// Run after Mobile Builder generation: node --test tests/nocode-*.test.cjs
...
const pageSource = read('_private/ionic/src/app/pages/page/page.ts');
const serviceSource = read('_private/ionic/src/app/services/actionbeans.service.ts');
...
function adapter(source, marker) {
  const selected = methods(source, (_, text) => text.includes(marker));
  assert.equal(selected.length, 1, marker);
  const method = selected[0];
  const name = method.match(/(?:public\s+)?(\w+)\s*\(/)[1];
  const Probe = run('class Probe { ' + method + ' } this.Probe = Probe;', { window: { clearInterval() {} } }).Probe;
  const probe = new Probe();
  return (p, vars) => probe[name](p, {}, vars, {});
}
const applyCatalog = adapter(serviceSource, 'var catalog = unwrap(vars.response);');
const resume = adapter(pageSource, '// Restore resource identity only.');
...
test('persisted/public context contains resource identity only, and Studio has none', () => {
  ...
  const ctx = run(snippets.join('\n'), {
    ...
    sanitizeCodexHome: () => '', now: () => 100, File: function(p) { this.path = p; }, writeJsonFile: (_, r) => { record = r; }, readState: () => null
  });
