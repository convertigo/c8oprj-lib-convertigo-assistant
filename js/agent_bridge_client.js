...
(function () {
  ...
  function buildSequencePrompt(rawQuestion, options) {
    ...
    if (simpleViewerFollowup) {
      ...
      promptParts.push("- Do not reread an unchanged managed `SKILL.md`, `convertigo://capabilities`, `convertigo://resources/convertigo-start`, or any guide already read in this conversation. If the Agent Bridge preflight reports a changed skill bundle, its exact re-read list takes precedence. Do not repeat catalog discovery.");
      ...
      promptParts.push("- When this turn changes NGX pages, components, styles, or events, start the viewer early: call `mobile-builder-open({project, wait:false})` before the first mutation so the builder warms up while you work, then do the readiness call and the browser proof before the final answer. Skip the early start only for advice-only or backend-only requests.");
    ...
    if (!isNoCodeSurface) {
      promptParts.push("- For frontend work (NGX pages, components, styles, events), start the Studio viewer early with `mobile-builder-open({project, wait:false})` as soon as the target project is known, before the first mutation: the builder warms up (Node, npm, Angular) while you inspect and edit. Then use one readiness call and a browser proof before claiming completion. Do not start it for advice-only or backend-only requests.");
    }
